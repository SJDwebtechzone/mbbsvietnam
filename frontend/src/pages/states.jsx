...
//   useEffect(() => {
//     fetch(`${BASE_URL}/api/top-universities/public`).then(r => r.json()).then(data => { setUniversities(Array.isArray(data) ? data : []); setUnisLoading(false); }).catch(() => { setUniversities([]); setUnisLoading(false); });
//   }, []);

//   useEffect(() => {
//     if (!d) return;
//     fetch(`${BASE_URL}/api/reviews?state=${encodeURIComponent(d.name)}`).then(r => r.json()).then(data => { setReviews(Array.isArray(data) ? data : []); setReviewsLoading(false); }).catch(() => { setReviews([]); setReviewsLoading(false); });
//   }, [d?.name]);
...
//           <h1 className="text-2xl font-bold" style={{ color: COLORS.heading }}>State Not Found</h1>
//           <p style={{ color: COLORS.textMain }}>We don't have a guide for <strong>{slug}</strong> yet.</p>
...
//   const waHref = `https://wa.me/91${d.counselorPhone.replace(/\D/g, "").slice(-10)}?text=${d.whatsappMessage}`;
...
//           <Link to="/" className="hover:text-white">Home</Link> / <Link to="/mbbsInVietnam" className="hover:text-white">MBBS in Vietnam</Link> / <span style={{ color: COLORS.accent }}>{d.breadcrumbLabel}</span>
//         </p>
//       </div>
...
//           <div>
//             <span className="inline-block px-3 py-1 rounded-full text-xs font-bold uppercase mb-3" style={{ background: COLORS.white, color: COLORS.accent }}>{d.heroBadge}</span>
//             <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold leading-tight mb-4" style={{ color: COLORS.heading }}>
//               Study MBBS in Vietnam<br />from <span style={{ color: COLORS.accent }}>{d.name}</span>
//             </h1>
//             <p className="text-base leading-relaxed mb-6 max-w-md" style={{ color: COLORS.textMain }}>{d.heroSubtitle}</p>
...
//               <a href={waHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-6 py-2.5 rounded-lg font-medium text-sm" style={{ border: `1px solid ${COLORS.border}`, color: COLORS.textMain }}><WaSvg /> WhatsApp Chat</a>
//             </div>
//           </div>
...
//           <SectionTitle tag="Why Vietnam?" title={`Why ${d.name} Students Choose`} highlight="Vietnam for MBBS" sub={`${d.neetCount}+ NEET takers from ${d.name} choose us every year.`} />
//           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
//             {d.whyPoints.map((p, i) => (
//               <div key={i} className="p-5 rounded-xl" style={{ background: COLORS.white, border: `1px solid ${COLORS.border}` }}>
//                 <div className="text-3xl mb-3">{p.icon}</div>
//                 <h3 className="font-bold text-base mb-1" style={{ color: COLORS.heading }}>{p.title}</h3>
//                 <p className="text-sm leading-relaxed" style={{ color: COLORS.textMain }}>{p.desc}</p>
//               </div>
//             ))}
//           </div>
...
//                 <p className="text-sm" style={{ color: COLORS.textMain }}>{s.label}</p>
//               </div>
//             ))}
//           </div>
//         </div>
...
//         <div className="max-w-7xl mx-auto">
//           <SectionTitle tag="Top Universities" title="Top Medical Universities" highlight="in Vietnam" sub={`NMC Approved universities preferred by students from ${d.name}.`} />
//           {universitiesLoading && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{Array(4).fill().map((_, i) => <CardSkeleton key={i} />)}</div>)}
//           {!universitiesLoading && universities.length === 0 && (<div className="text-center py-12"><div className="text-5xl mb-3">🎓</div><p className="text-sm">University listings coming soon. Contact us for details.</p></div>)}
//           {!universitiesLoading && universities.length > 0 && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{universities.map((u, i) => (<UniversityCard key={u.id || i} u={u} onApply={() => setEnquireOpen(true)} />))}</div>)}
//           <p className="text-center text-sm mt-6">Students from {d.name} have successfully joined these universities. <button onClick={() => setEnquireOpen(true)} className="font-medium hover:underline" style={{ color: COLORS.accent }}>Talk to counselor</button></p>
//         </div>
//       </section>
...
//           <SectionTitle tag="Cost Comparison" title={`How Much Can ${d.name} Students`} highlight="Save?" sub="Compare total cost of MBBS in Vietnam vs private Indian colleges." />
...
//             <p className="text-sm uppercase tracking-wide mb-1" style={{ color: COLORS.textMain }}>Total Savings for {d.name} Students</p>
...
//       <section className="py-16 px-6" style={{ background: COLORS.smoked }}>
//         <div className="w-full">
//           <SectionTitle tag="Simple Process" title="Admission Process for" highlight={`${d.name} Students`} sub="From application to departure, we handle everything." />
...
//                   </div>
//                   <p className="text-sm leading-relaxed mt-1 whitespace-pre-line" style={{ color: COLORS.textMain }}>{s.desc}</p>
//                 </div>
//               </div>
//             ))}
//           </div>
//           <div className="text-center mt-8">
//             <button onClick={() => setEnquireOpen(true)} className="px-8 py-3 rounded-xl text-white font-semibold text-base" style={{ background: COLORS.accent }}>{d.applicationBtnText}</button>
//             <p className="text-xs mt-2">{d.applicationSubText}</p>
//           </div>
//         </div>
//       </section>
...
//       <section className="py-16 px-6" style={{ background: COLORS.white }}>
//         <div className="max-w-7xl mx-auto">
//           <SectionTitle tag="Travel Guide" title={`Easy Travel from ${d.name}`} highlight="to Vietnam" sub={`Convenient flight connections from ${d.capital}.`} />
//           <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
//             {d.travelCards.map((c, i) => (
...
//             <p className="text-sm font-medium" style={{ color: COLORS.accent }}>100% Visa Success Rate for {d.name} Students</p>
//           </div>
//         </div>
//       </section>
...
//       <section className="py-16 px-6" style={{ background: COLORS.smoked }}>
//         <div className="max-w-7xl mx-auto">
//           <SectionTitle tag="Testimonials" title={`Hear from ${d.name} Students`} highlight="in Vietnam" sub="Real experiences from students pursuing MBBS in Vietnam." />
//           {reviewsLoading && (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">{Array(3).fill().map((_, i) => (<div key={i} className="h-36 rounded-xl animate-pulse" style={{ background: COLORS.white, border: `1px solid ${COLORS.border}` }} />))}</div>)}
//           {!reviewsLoading && reviews.length === 0 && (<div className="text-center py-12"><div className="text-5xl mb-3">💬</div><p className="text-sm">Student testimonials coming soon!</p></div>)}
//           {!reviewsLoading && reviews.length > 0 && (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">{reviews.map((t, i) => (<div key={i} className="p-5 rounded-xl" style={{ background: COLORS.white, border: `1px solid ${COLORS.border}` }}>
...
//             <div className="flex gap-0.5 mb-2 text-sm" style={{ color: COLORS.accent }}>{Array(Number(t.rating) || 5).fill().map((_, j) => (<span key={j}>★</span>))}</div>
...
//           <SectionTitle tag="FAQs" title="Common Questions from" highlight={`${d.name} Students`} sub="Get answers about studying MBBS in Vietnam." />
...
//           <h2 className="font-bold text-2xl mb-2" style={{ color: COLORS.heading }}>{d.ctaTitle}</h2>
//           <p className="text-sm mb-5" style={{ color: COLORS.textMain }}>{d.ctaSubtitle}</p>
...
//       <Footer />
//       <button onClick={() => setEnquireOpen(true)} className="fixed bottom-5 left-5 z-50 flex items-center gap-1.5 px-3 py-2 rounded-full text-white text-xs shadow-md" style={{ background: COLORS.accent }}>📞 Enquire</button>
...
//                 <div><p className="text-xs font-bold text-white/80">Free Counseling</p><h3 className="font-bold text-lg text-white">Enquire for {d.name}</h3></div>
//                 <button onClick={() => setEnquireOpen(false)} className="text-white/80 hover:text-white text-xl">×</button>
//               </div>
//             </div>
//             <div className="p-5 space-y-3">
//               {["Full Name", "Phone Number", "Email Address"].map((ph) => (<input key={ph} placeholder={ph} className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ border: `1px solid ${COLORS.border}`, color: COLORS.heading }} onFocus={(e) => (e.currentTarget.style.border = `1px solid ${COLORS.accent}`)} onBlur={(e) => (e.currentTarget.style.border = `1px solid ${COLORS.border}`)} />))}
//               <select className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ border: `1px solid ${COLORS.border}`, color: COLORS.textMain }}><option>Select NEET Year</option><option>2024</option><option>2025</option><option>2026</option></select>
...
function SectionTitle({ tag, title, highlight, sub }) {
  return (
    <div className="text-center mb-12">
      ...
      >
        {title}{" "}
        {highlight && <span style={{ color: COLORS.accent }}>{highlight}</span>}
      </h2>
      {sub && (
        <p
          className="text-base max-w-2xl mx-auto mt-3 leading-relaxed"
  ...
  return (
    ...
    >
      <button
        ...
        style={{ background: open ? `${COLORS.accent}0A` : COLORS.white }}
      >
        <span
          className="font-semibold text-base"
          style={{ color: open ? COLORS.accent : COLORS.heading }}
        >
          {q}
        </span>
        <span
          ...
          style={
            open
              ...
              : { background: COLORS.majorBg, color: COLORS.textMain }
          }
        >
          {open ? "−" : "+"}
        </span>
      ...
      {open && (
        <div
          className="px-5 pb-4 pt-2 text-sm leading-relaxed"
          style={{
            background: COLORS.majorBg,
            color: COLORS.textMain,
    ...
    >
      <div className="h-44 relative overflow-hidden" style={{ background: COLORS.accent }}>
        ...
        <span
          ...
          style={{ background: COLORS.white, color: COLORS.accent }}
        >
          NMC Approved
        </span>
        {u.location && (
          <span
            className="absolute bottom-2 left-2 px-2 py-0.5 rounded-full text-[10px] font-medium"
            style={{ background: "rgba(0,0,0,0.6)", color: COLORS.white }}
          >
            📍 {u.location}
          </span>
        )}
      ...
      <div className="p-4">
        <h3 className="font-bold text-base leading-tight mb-2" style={{ color: COLORS.heading }}>
          {u.name}
        </h3>
        <div
          className="rounded-lg px-3 py-2 mb-3 flex items-baseline gap-2"
          style={{ background: COLORS.majorBg }}
        >
        ...
        <div className="flex gap-2">
          <Link
            ...
            style={{ border: `1px solid ${COLORS.border}`, color: COLORS.textMain }}
          >
            View Details
          </Link>
          <button
  ...
  return (
    ...
    >
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="h-10 rounded" style={{ background: COLORS.majorBg }} />
        <div className="flex gap-2">
          <div className="flex-1 h-9 rounded" style={{ background: COLORS.border }} />
          <div className="flex-1 h-9 rounded" style={{ background: `${COLORS.accent}33` }} />
        </div>
      </div>
    </div>
  );
...
//   /admission-guides/:slug          → state slug  (e.g. "tamil-nadu")
//   /admission-guides/city/:citySlug → city slug   (e.g. "chennai")
// ─────────────────────────────────────────────────────────────────────────────
export default function AdmissionGuide() {
  // Both route params are available; only one will be defined per render
  const { slug, citySlug } = useParams();

  // ── Resolve which state data to use & what "display name" to show ──
  let stateSlug = null;
  let displayName = null;   // What we show in headings  (e.g. "Chennai" or "Tamil Nadu")
  let isCity = false;

  if (citySlug) {
    // City route: /admission-guides/city/:citySlug
    isCity = true;
    stateSlug = cityToState[citySlug] || null;
    displayName = cityNameFromSlug(citySlug);
  } else {
    // State route: /admission-guides/:slug
    stateSlug = slug;
    displayName = null; // will be taken from d.name below
  }

  const d = stateSlug ? statesData[stateSlug] : null;

  // If it's a city page, displayName is the city name; otherwise use state name
  const locationName = isCity ? displayName : d?.name;
  ...
  useEffect(() => {
    if (!d) return;
    fetch(`${BASE_URL}/api/reviews?state=${encodeURIComponent(d.name)}`)
      .then((r) => r.json())
      .then((data) => {
      ...
      <div className="font-sans">
        ...
        >
          ...
          <h1 className="text-2xl font-bold" style={{ color: COLORS.heading }}>
            {isCity ? "City Not Found" : "State Not Found"}
          </h1>
          <p style={{ color: COLORS.textMain }}>
            We don't have a guide for{" "}
            <strong>{isCity ? citySlug : slug}</strong> yet.
          </p>
          <Link
            to="/"
  ...
  const waHref = `https://wa.me/91${d.counselorPhone
    .replace(/\D/g, "")
    .slice(-10)}?text=${d.whatsappMessage}`;

  // ── Build breadcrumb label ──
  // For city pages: "Chennai Students" instead of "Tamil Nadu Students"
    ...
    <div className="font-sans" style={{ background: COLORS.smoked, color: COLORS.textMain }}>
      ...
      <div style={{ background: COLORS.heading, padding: "10px 24px" }}>
        <p className="text-sm max-w-7xl mx-auto text-white/60">
          ...
          /{" "}
          <Link to="/mbbsInVietnam" className="hover:text-white">
            MBBS in Vietnam
          </Link>{" "}
          /{" "}
          <span style={{ color: COLORS.accent }}>{breadcrumbLabel}</span>
        </p>
      </div>
      ...
      <section
        ...
        style={{ background: COLORS.smoked, borderBottom: `1px solid ${COLORS.border}` }}
      >
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div>
            <span
            ...
            >
              ...
              <br />
              from{" "}
              <span style={{ color: COLORS.accent }}>{locationName}</span>
            </h1>
            <p
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              <a
                ...
                className="inline-flex items-center gap-1.5 px-6 py-2.5 rounded-lg font-medium text-sm"
                style={{ border: `1px solid ${COLORS.border}`, color: COLORS.textMain }}
              >
                <WaSvg /> WhatsApp Chat
              </a>
        ...
        {/* Stats bar */}
        <div
          className="max-w-7xl mx-auto mt-8 pt-6 border-t"
      ...
      <section className="py-16 px-6" style={{ background: COLORS.white }}>
        <div className="max-w-7xl mx-auto">
          <SectionTitle
            tag="Why Vietnam?"
            title={`Why ${locationName} Students Choose`}
            highlight="Vietnam for MBBS"
            sub={`${d.neetCount}+ NEET takers from ${isCity ? d.name : locationName} choose us every year.`}
          />
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {d.whyPoints.map((p, i) => (
              <div
              ...
              >
                ...
                <p className="text-sm leading-relaxed" style={{ color: COLORS.textMain }}>
                  {p.desc}
                </p>
              </div>
            ))}
          ...
          {/* Highlight stats */}
          <div
            className="mt-10 grid grid-cols-3 rounded-xl overflow-hidden"
            style={{ background: COLORS.white, border: `1px solid ${COLORS.border}` }}
          >
            {d.highlightStats.map((s, i) => (
              <div
              ...
              >
                <p className="font-bold text-2xl" style={{ color: COLORS.accent }}>
                  {s.val}
                </p>
      ...
      <section className="py-16 px-6" style={{ background: COLORS.smoked }}>
        <div className="max-w-7xl mx-auto">
          <SectionTitle
            ...
            sub={`NMC Approved universities preferred by students from ${locationName}.`}
          />
          {universitiesLoading && (
          ...
          {!universitiesLoading && universities.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm">
                University listings coming soon. Contact us for details.
              </p>
            </div>
          )}
          {!universitiesLoading && universities.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {universities.map((u, i) => (
                <UniversityCard
                  key={u.id || i}
                  u={u}
                  onApply={() => setEnquireOpen(true)}
                />
              ))}
            </div>
          )}
          <p className="text-center text-sm mt-6">
            Students from {locationName} have successfully joined these universities.{" "}
            <button
              onClick={() => setEnquireOpen(true)}
      ...
      <section className="py-16 px-6" style={{ background: COLORS.white }}>
        <div className="w-full">
          <SectionTitle
            tag="Cost Comparison"
            title={`How Much Can ${locationName} Students`}
            highlight="Save?"
          ...
          >
            <div
              className="grid grid-cols-3 text-center text-white text-sm font-semibold"
              style={{ background: COLORS.heading }}
            >
              <div className="py-3 px-2">Expense Category</div>
              <div className="py-3 px-2">MBBS in Vietnam</div>
              <div className="py-3 px-2">Private India</div>
            ...
            {d.costRows.map(([cat, vn, ind], i) => (
              <div
                ...
                style={{
                  borderTop: `1px solid ${COLORS.border}`,
                  background: i % 2 === 0 ? COLORS.white : COLORS.majorBg,
                }}
              >
                <div className="py-2.5 px-2" style={{ color: COLORS.textMain }}>
                  {cat}
                </div>
                <div className="py-2.5 px-2 font-medium" style={{ color: COLORS.accent }}>
                  {vn}
                </div>
                <div className="py-2.5 px-2" style={{ color: COLORS.muted }}>
                  {ind}
                </div>
              </div>
            ))}
            <div
              className="grid grid-cols-3 text-center font-bold"
              style={{
                borderTop: `2px solid ${COLORS.accent}`,
                background: COLORS.majorBg,
              }}
            >
              <div className="py-2.5 px-2 text-sm" style={{ color: COLORS.heading }}>
                Total Cost (6 Years)
              </div>
              <div className="py-2.5 px-2 text-base" style={{ color: COLORS.accent }}>
                {d.totalVietnam}
              </div>
              <div
                className="py-2.5 px-2 text-sm line-through"
          ...
          <div
            ...
            style={{ background: COLORS.smoked }}
          >
            <p
              className="text-sm uppercase tracking-wide mb-1"
              style={{ color: COLORS.textMain }}
            >
              Total Savings for {locationName} Students
            </p>
            <p className="text-3xl md:text-4xl font-bold" style={{ color: COLORS.accent }}>
              {d.savings}
            </p>
            <p className="text-sm mt-1">
              That's <strong>{d.savingsPercent} cheaper</strong> than private Indian colleges!
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-6" style={{ background: COLORS.smoked }}>
        <div className="w-full">
          <SectionTitle
            ...
            highlight={`${locationName} Students`}
          ...
          <div className="flex flex-col gap-4">
            {d.steps.map((s, i) => (
              <div
                key={i}
                className="flex gap-4 items-start p-5 rounded-xl w-full"
                style={{ background: COLORS.white, border: `1px solid ${COLORS.border}` }}
              >
                <div
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap justify-between items-start gap-2">
                    <h3 className="font-bold text-base" style={{ color: COLORS.heading }}>
                      {s.title}
                    </h3>
                    <span
                  ...
                  <p
                    className="text-sm leading-relaxed mt-1 whitespace-pre-line"
                    style={{ color: COLORS.textMain }}
                  >
                    {s.desc}
                  </p>
      ...
      <section className="py-16 px-6" style={{ background: COLORS.white }}>
        <div className="max-w-7xl mx-auto">
          <SectionTitle
            tag="Travel Guide"
            title={`Easy Travel from ${locationName}`}
            highlight="to Vietnam"
            sub={`Convenient flight connections from ${isCity ? locationName : d.capital}.`}
          />
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {d.travelCards.map((c, i) => (
              <div
          ...
          <div
            ...
            style={{ background: COLORS.white, border: `1px solid ${COLORS.accent}` }}
          >
            <span className="text-xl">✅</span>
            <p className="text-sm font-medium" style={{ color: COLORS.accent }}>
              100% Visa Success Rate for {locationName} Students
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-6" style={{ background: COLORS.smoked }}>
        <div className="max-w-7xl mx-auto">
          <SectionTitle
            tag="Testimonials"
            title={`Hear from ${isCity ? d.name : locationName} Students`}
            highlight="in Vietnam"
            sub="Real experiences from students pursuing MBBS in Vietnam."
          />
          {reviewsLoading && (
          ...
          {!reviewsLoading && reviews.length === 0 && (
            <div className="text-center py-12">
              <div className="text-5xl mb-3">💬</div>
              <p className="text-sm">Student testimonials coming soon!</p>
            </div>
          )}
          {!reviewsLoading && reviews.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {reviews.map((t, i) => (
                ...
                >
                  ...
                  <p className="text-sm font-bold" style={{ color: COLORS.accent }}>
                    {t.name}
                    <span className="font-normal" style={{ color: COLORS.textMain }}>
                      {" "}
                      · {t.city}
                    </span>
                  </p>
      ...
      <section className="py-16 px-6" style={{ background: COLORS.white }}>
        <div className="max-w-3xl mx-auto">
          <SectionTitle
            ...
            highlight={`${locationName} Students`}
            sub="Get answers about studying MBBS in Vietnam."
          />
          {d.faqs.map((f, i) => (
      ...
      <section className="py-12 px-6 text-center" style={{ background: COLORS.smoked }}>
        <div className="max-w-xl mx-auto">
          ...
          <h2 className="font-bold text-2xl mb-2" style={{ color: COLORS.heading }}>
            {isCity
              ? `Ready to Start Your MBBS Journey from ${locationName}?`
              : d.ctaTitle}
          </h2>
          <p className="text-sm mb-5" style={{ color: COLORS.textMain }}>
            {isCity
              ? `Free counseling for students from ${locationName} and across ${d.name}`
              : d.ctaSubtitle}
          </p>
          <div className="flex justify-center gap-3 flex-wrap">
            <button
            ...
            <a
              href={waHref}
              target="_blank"
              className="inline-flex items-center gap-1.5 px-6 py-2.5 rounded-xl text-white text-sm"
      ...
      <button
        onClick={() => setEnquireOpen(true)}
        className="fixed bottom-5 left-5 z-50 flex items-center gap-1.5 px-3 py-2 rounded-full text-white text-xs shadow-md"
        style={{ background: COLORS.accent }}
      >
        📞 Enquire
      </button>
            ...
            <div className="p-5 space-y-3">
              {["Full Name", "Phone Number", "Email Address"].map((ph) => (
                <input
                  ...
                  style={{ border: `1px solid ${COLORS.border}`, color: COLORS.heading }}
                  onFocus={(e) =>
                    (e.currentTarget.style.border = `1px solid ${COLORS.accent}`)
                  }
                  onBlur={(e) =>
              ...
              <p className="text-center text-xs pt-1">
                Or reach us on{" "}
                <a href={waHref} target="_blank" style={{ color: COLORS.whatsapp }}>
                  WhatsApp
                </a>
              </p>
            </div>
